...
function UpdateTrainingStatusModal({ isOpen, onClose, onSuccess, training }) {
  ...
  useEffect(() => {
    ...
    setStatus(training.status ?? 'Scheduled')
    setSubmitting(false)
    setError('')
  }, [isOpen, training])
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.patch(`/hr/training/${training.id}/status`, { status })
      await onSuccess?.()
      onClose()
    } catch (requestError) {
      const message =
        ...
        'Unable to update this training program right now.'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-xl rounded-[2rem] bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Keep the company training calendar accurate as programs progress.
            </p>
          </div>
        ...
        <div className="mt-6 rounded-3xl bg-blue-50 px-5 py-4 text-sm text-blue-700">
          Updating <span className="font-semibold">{training.title ?? 'this training'}</span>.
        </div>

        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="training-status">
              Status
            </label>
            <select
              id="training-status"
