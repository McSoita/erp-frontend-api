...
const payrollRunStatuses = ['Draft', 'In Progress', 'Completed', 'Cancelled']
...
function UpdatePayrollRunStatusModal({ isOpen, onClose, onSuccess, payrollRun }) {
  ...
  useEffect(() => {
    ...
    setStatus(payrollRun.status ?? 'Draft')
    setSubmitting(false)
    setError('')
  }, [isOpen, payrollRun])
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.patch(`/hr/payroll/runs/${payrollRun.id}/status`, { status })
      await onSuccess?.()
      onClose()
    } catch (requestError) {
      setError(
        requestError.response?.data?.message ??
          ...
          'Unable to update payroll run status. Please try again.',
      )
    } finally {
      setSubmitting(false)
    }
      ...
      <div className="w-full max-w-xl rounded-[2rem] bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Adjust the processing state for {payrollRun.pay_period ?? 'this payroll run'}.
          </p>
        </div>
...
export default UpdatePayrollRunStatusModal
