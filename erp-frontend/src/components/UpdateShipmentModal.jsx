...
function UpdateShipmentModal({ isOpen, onClose, onSuccess, shipment }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.patch(`/scm/shipments/${shipment.id}/status`, { status })
      ...
      <div className="w-full max-w-xl rounded-[2rem] bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Adjust the live logistics state for tracking number{' '}
              {shipment.tracking_number ?? 'Unassigned shipment'}.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          <div>
            ...
            >
              {shipmentStatuses.map((statusOption) => (
