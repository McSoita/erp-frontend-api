...
function UpdateOpportunityStageModal({ isOpen, onClose, onSuccess, opportunity }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.patch(`/crm/opportunities/${opportunity.id}/stage`, { stage })
      await onSuccess?.()
      onClose()
    } catch (requestError) {
      const message =
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-xl rounded-[2rem] bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Move {opportunity.title ?? 'this opportunity'} to its next commercial stage.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          <div>
            <label htmlFor="opportunity-stage" className="mb-2 block text-sm font-medium text-slate-700">
              Stage
            </label>
            <select
...
export default UpdateOpportunityStageModal
