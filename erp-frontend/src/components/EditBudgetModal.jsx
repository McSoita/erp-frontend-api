...
function EditBudgetModal({ isOpen, onClose, onSuccess, budget }) {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      allocatedAmount: String(budget.allocated_amount ?? ''),
      spentAmount: String(budget.spent_amount ?? ''),
    })
    setSubmitting(false)
    setError('')
  }, [budget, isOpen])
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.patch(`/finance/reports/budgets/${budget.id}`, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-[2rem] bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold text-slate-900">Edit Budget</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              <label htmlFor="allocatedAmount" className="mb-2 block text-sm font-medium text-slate-700">
                Allocated Amount
              </label>
              <input
                id="allocatedAmount"
