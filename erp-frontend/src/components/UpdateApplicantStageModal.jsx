...
const stageOptions = ['Applied', 'Screening', 'Interviewing', 'Offered', 'Hired', 'Rejected']
...
function getApplicantName(applicant) {
  return [applicant?.first_name, applicant?.last_name].filter(Boolean).join(' ') || 'Applicant'
}

function UpdateApplicantStageModal({
  isOpen,
  onClose,
  onSuccess,
  applicant,
  ...
  useEffect(() => {
    ...
    setPipelineStage(applicant.pipeline_stage ?? 'Applied')
    setSubmitting(false)
    setError('')
  }, [applicant, isOpen])
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.patch(`/hr/recruitment/applicants/${applicant.id}/stage`, {
      ...
      const message =
        ...
        'Unable to update this applicant right now.'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-xl rounded-[2rem] bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Move this candidate through the hiring pipeline.
            </p>
          </div>
        ...
        <div className="mt-6 rounded-3xl bg-blue-50 px-5 py-4 text-sm text-blue-700">
          Updating <span className="font-semibold">{getApplicantName(applicant)}</span> for {applicant.job_title ?? 'this role'}.
        </div>

        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="pipeline-stage">
              Pipeline Stage
            </label>
            <select
              id="pipeline-stage"
...
export default UpdateApplicantStageModal
