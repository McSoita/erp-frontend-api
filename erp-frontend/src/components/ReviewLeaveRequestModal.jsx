...
function getEmployeeName(request) {
  return [request?.first_name, request?.last_name].filter(Boolean).join(' ') || 'Employee'
}

function ReviewLeaveRequestModal({ isOpen, onClose, onSuccess, request }) {
  ...
  useEffect(() => {
    ...
    setStatus(request.status === 'Rejected' ? 'Rejected' : 'Approved')
    setSubmitting(false)
    setError('')
  }, [isOpen, request])
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await apiClient.patch(`/hr/leave/${request.id}/status`, { status })
      await onSuccess?.()
      onClose()
    } catch (requestError) {
      const message =
        ...
        'Unable to review this leave request right now.'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-xl rounded-[2rem] bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Update the approval status for this time-off request.
            </p>
          </div>
        ...
        <div className="mt-6 rounded-3xl bg-blue-50 px-5 py-4 text-sm text-blue-700">
          ...
          {` requested ${request.leave_type ?? 'leave'} from ${request.start_date ?? 'N/A'} to ${request.end_date ?? 'N/A'}.`}
        </div>
